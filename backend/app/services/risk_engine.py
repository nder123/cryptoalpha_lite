...
class RiskEngine:
    ...
    async def run(self, stop_event: asyncio.Event) -> None:
        async for message in self._bus.listen(
            ...
            assessment = self._assess_hypothesis(hypothesis, current_exposure, risk_budget, daily_stats)
    ...
    async def _compute_daily_limits(self) -> Dict[str, float]:
        ...
        async with db_session() as session:
            ...
            pnl_today = (
                await session.execute(
                    select(func.coalesce(func.sum(TradeSession.pnl_usdt), 0)).where(*base_filters)
                )
            ).scalar_one()
    ...
    def _assess_hypothesis(
        ...
        consecutive_losses = int(self._safe_float(daily_stats.get("consecutive_losses")))

        metrics["trades_today"] = trades_today
        metrics["pnl_today"] = pnl_today
        metrics["consecutive_losses"] = float(consecutive_losses)

        if self._config.max_trades_per_day and trades_today >= float(self._config.max_trades_per_day):
            blockers.append("exceeds max trades per day")

        if self._config.max_daily_loss_usdt and pnl_today <= -abs(float(self._config.max_daily_loss_usdt)):
            blockers.append("exceeds daily loss limit")
            metrics["max_daily_loss_usdt"] = float(self._config.max_daily_loss_usdt)

        if self._config.max_consecutive_losses and consecutive_losses >= int(self._config.max_consecutive_losses):
            blockers.append("exceeds max consecutive losses")
            metrics["max_consecutive_losses"] = float(self._config.max_consecutive_losses)
        ...
        allocation_limit = symbol_limit if symbol_limit > 0 else effective_portfolio_limit * self._config.max_symbol_allocation_pct
        ...
        limit = effective_portfolio_limit if effective_portfolio_limit > 0 else self._config.max_portfolio_exposure_usdt
