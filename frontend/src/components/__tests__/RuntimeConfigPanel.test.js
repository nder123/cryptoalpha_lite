...
describe('RuntimeConfigPanel', () => {
    let onSubmit;
    beforeEach(() => {
        onSubmit = vi.fn(async (_payload) => undefined);
    });
    it('renders loading state without config', () => {
        render(_jsx(RuntimeConfigPanel, { config: null, saving: false, onSubmit: onSubmit }));
        expect(screen.getByText('Загрузка текущих настроек…')).toBeInTheDocument();
    });
    it('disables submit when nothing changed', () => {
        render(_jsx(RuntimeConfigPanel, { config: baseConfig, saving: false, onSubmit: onSubmit }));
        const submit = screen.getByRole('button', { name: 'Сохранить' });
        expect(submit).toBeDisabled();
    });
    it('toggles auto research and submits payload', async () => {
        render(_jsx(RuntimeConfigPanel, { config: baseConfig, saving: false, onSubmit: onSubmit }));
        const toggle = screen.getByRole('button', { name: 'Переключить auto-research' });
