...
const DEFAULT_FILTERS: ExchangeTransactionFilters = {
    start: null,
    end: null,
    txType: '',
};

export function useExchangeTransactions(limit: number = 100) {
    const [filters, setFilters] = useState<ExchangeTransactionFilters>(DEFAULT_FILTERS);
    const [records, setRecords] = useState<AccountTransactionEntry[]>([]);
    ...
    const updateFilters = useCallback((patch: Partial<ExchangeTransactionFilters>) => {
